...
{ name={Resistor},
  description={A component which opposes current flow, causing loss as heat},
  %sort={0}
  }
...
\newglossaryentry{srs}
{ name={Series},
  description={It is a circuit configuration where current passes through each component one after another},
  %sort={0}
  }

\newglossaryentry{prl}
{ name={Parallel},
  description={It is a circuit configuration where a circuit board/node splits into several branches, with current being divided inversely by a ratio of each branches resistance. It can also stand for a communication bus of multiple concurrent data lines},
  %sort={0}
  }

\newglossaryentry{ptldvdr}
{ name={Potential Divider},
  description={A resistor configuration used to get an output which is a fraction of a voltage. Widely used in sensing},
  %sort={0}
  }
...
\newglossaryentry{rms}
{ name={RMS},
  description={This is an acronym for Root Mean Squared. This is the effective value of a waveform, for example the DC magnitude equivlane energy wise},
  %sort={0}
  }
...
\newglossaryentry{led}
{ name={LED},
  description={This stands for Light Emitting Diode, which are diodes doped so that they emit light, typically in the visible spectrum, depening on use case},
  %sort={0}
  }
...
{ name={Transformer},
  description={A device which will increase or decrease the voltage of an alternating current source, having the reverse effect upon current},
  %sort={0}
  }
...
{ name={Lamination},
  description={It is the process of using stacked sheets of core material rather than a single "block" to limit eddy current},
  %sort={0}
  }
...
{ name={Transistor},
  description={A semiconductor meant for electronics control, signal generation or amplification},
  %sort={0}
  }

\newglossaryentry{gte}
{ name={Gate},
  description={It can be thought of as a trigger to allow current flow in a transistor. Where in boolean algebra, it is an idealized device implementing a Boolean function},
  %sort={0}
  }

\newglossaryentry{bjt}
{ name={BJT},
  description={It is short for Bipolar Junction \gls{trstr}, where the current flow between the collector and emitter is the current flow through the base amplified},
  %sort={0}
  }
...
{ name={Darlington Pair},
  description={A \gls{trstr} which internally uses a pair of BJT \gls{trstr}s to allow for higher current amplification than a single BJT},
  %sort={0}
  }
...
{ name={MEMS},
  description={This stands for MicroElectroMechanical Systems, and is a combination of micromechanical components such as springs or levers, along with microelectronic circuitry, such as capacitors, widely used for sensors},
  %sort={0}
  }

\newglossaryentry{thrmcpl}
{ name={Thermocouple},
  description={A sensor for measuring temperature, based on the voltage developed between two junctions, proportional to temperature difference},
  %sort={0}
  }

\newglossaryentry{rtd}
{ name={RTD},
  description={This stands for Resistance Temperature Detector and is a sensor which has its resistance vary significantly with temperature. Also referred to as a resistance thermometer},
  %sort={0}
  }

\newglossaryentry{thrmstr}
{ name={Thermistor},
  description={A resistor whose resistance alters with temperature change, commonly used as a sensor},
  %sort={0}
  }
...
{ name={LiDAR},
  description={This stands for light detection and ranging, using a laser to measure distance by the time it takes to reflect back to the sensor. This method can be known as ToF (Time of Flight)},
  %sort={0}
  }
...
{ name={Supercapacitor},
  description={A form of capacitor, with far higher energy density, not far off standard batteries, ideal for high power scenarios, due to their low resistance},
  %sort={0}
  }
...
{ name={Reactance},
  description={This is due to a combination of inductance and capacitance, and makes up the non-reactive part of impedance, therefore AC specific},
  %sort={0}
  }
...
\newglossaryentry{esd}
{ name={ESD},
  description={This stands for Electrostatic Discharge, and is the sudden flow of current between two electrically charged objects, typically through contact. Steps should be taken to avoid this, which can cause damage},
  %sort={0}
  }
...
{ name={Multimeter},
  description={A common piece of test equipment typically for measuring current, voltage and resistance, sometimes with extra functions such as capacitance or transistor current gain measurement},
  %sort={0}
  }

\newglossaryentry{osclscp}
{ name={Oscilloscope},
  description={A piece of equipment for measuring and visualising voltage or current against time, such as for viewing oscillations},
  %sort={0}
  }
...
\caption{LabVIEW Capacitor Circuit Block Diagram Initial Setup}
\end{figure}
...
\caption{LabVIEW Capacitor Circuit Block Diagram Read and Write Blocks}
\end{figure}
...
\caption{LabVIEW Capacitor Circuit Block Diagram Input Output Channels}
\end{figure}
...
\caption{LabVIEW Capacitor Circuit Block Diagram Error/Stop Handling}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{screenshots/labviewcap5}
\caption{LabVIEW Capacitor Circuit Stop Button Front Panel}
\end{figure}
...
\caption{LabVIEW Capacitor Circuit Block Diagram Final}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{screenshots/labviewcap7}
\caption{LabVIEW Capacitor Circuit Front Panel Final}
\end{figure}
...
\caption{LabVIEW Case Based LED Final Block Diagram}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{screenshots/labviewcaseled2}
\caption{LabVIEW Case Based LED Final Front Panel}
\end{figure}
